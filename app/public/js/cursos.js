      ...
      courseContainer.appendChild(certBtn);
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function renderCursos(Recursos) {
                ...
                Recursos.forEach(recurso => {
                    ...
                    card.innerHTML = `
                        ...
                        <button class="action-button" onclick="window.open('recursos/${recurso.ubicacion}', '_blank')">Ver más</button>
            ...
            async function cargarCursos() {
                try {
                    const res = await fetch(`/api/recurso`); // Ajusta la URL según tu backend
                    const data = await res.json();
                    renderCursos(data);
                } catch (error) {
